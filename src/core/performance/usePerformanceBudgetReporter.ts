...
import { DefaultBudgetConfig } from './budget.config';
...
export function usePerformanceBudget(options: UsePerformanceBudgetOptions = {}): {
  ...
  useEffect(() => {
    const updateState = () => {
      ...
      setState((prev) => ({
        ...
        lastUpdate: Date.now(),
        stats: {
          totalMetrics: Object.values(prev.metrics).reduce(
  ...
  const getWorstMetric = (breaches: PerformanceBreach[]): string | null => {
    ...
    const breachCounts = breaches.reduce(
      (acc, breach) => {
    ...
    return Object.entries(breachCounts).sort(([, a], [, b]) => b - a)[0]?.[0] || null;
  };
  ...
  const actions: PerformanceBudgetActions = {
    ...
    recordMetric: useCallback(
      (name: string, value: number, category: string, metadata?: Record<string, unknown>) => {
        if (monitorRef.current) {
...
export function usePerformanceAlerts() {
  ...
  useEffect(() => {
    ...
    setAlerts((prev) => {
      ...
      return [...prev, ...uniqueNewAlerts].sort((a, b) => b.timestamp - a.timestamp).slice(0, 10); // Keep only 10 most recent alerts
    });
  }, [state.breaches]);
